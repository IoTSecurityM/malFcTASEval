...
class DtStrRFEDFrank(object):
    def __init__(self, annotations_file, mal_dir, dtanno_file):
        ...
        self.fea_filter = "../../Features/StrRFEDFrank/SamSpa/StrRFEDFrank"+self.dtanno_file[0:-5]+".xlsx"
        self.dt_str_infor = "../../Features/StrRFEDFrank/SamSpa/stringInforStrRFEDFrank"+self.dtanno_file[0:-5]+".txt"
        self.top50_str_perfam = "../../Features/StrRFEDFrank/SamSpa/to50strperfam"+self.dtanno_file[0:-5]+".txt" 
        ...
        with pd.ExcelWriter(self.fea_filter, engine='xlsxwriter') as writer:
            ...
            for train_year, infor in self.top50_str_infor.items():
                ...
                for arch, arch_infor in infor.items():
                    ...
                    current_row = 1                  
                    reorg_test_mal_labels = self.mal_labels[~((self.mal_labels['year']==train_year) & (self.mal_labels['arch']==arch))]
                    print(f"original len {len(self.mal_labels)}, filtered len {len(reorg_test_mal_labels)}")
                    reorg_test_mal_labels = pd.concat([reorg_test_mal_labels, self.test_mal_labels[(self.test_mal_labels['year']==train_year)&(self.test_mal_labels['arch']==arch)]], axis=0, ignore_index=True)                
                    print(f"updated len {len(reorg_test_mal_labels)}")
                    for index, row in reorg_test_mal_labels.iterrows():
                        mal_path = self.mal_dir + row[0]              
                        fea = strrfedfrank.get_features(mal_path)                   
                        whole_fea = [row[0], row[1], row[2], row[4]] + fea 
...
def generate_fea():
    ...
    train_dtanno_dir = '/home/yefei/WSPACE/DtComb/IoTXPOT/DtCSV/'
    mal_dir = '/home/yefei/WSPACE/DtComb/IoTXPOT/' 
