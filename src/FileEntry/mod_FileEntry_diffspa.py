...
def optimal_results(dtanno_file):
    fea_filter = "../../Features/FileEntry/DiffSpa/FileEntry"+dtanno_file[0:-5]+".xlsx" 
    ...
    for train_year, train_arch in itertools.product(year_list, train_arch_list):
        ...
        param_grid_lsvc = {
            ...
            'linear_svc__max_iter': [2000, 5000, 8000],  # Increase the number of iterations
        ...
        knn_pipeline = Pipeline([
            ('scaler', StandardScaler(with_mean=False)),  # Apply StandardScaler to KNN
            ('knn', KNeighborsClassifier())
        ])      
        lsvc_pipeline = Pipeline([
            ('scaler', StandardScaler(with_mean=False)),  # StandardScaler to normalize the data
        ...
        print(f"{sheetname}:")
        sheetname_best_model_dir = "../../Results/BestModels/FileEntry/FileEntry"+dtanno_file[0:-5]+"/"+sheetname
        if not os.path.exists(sheetname_best_model_dir):
            os.makedirs(sheetname_best_model_dir)
        
        for name, (model, param_grid) in models.items():
            # Train the model           
            grid_search = GridSearchCV(estimator=model, param_grid=param_grid, cv=4, scoring='f1_macro', n_jobs=-1)
    ...
    result_path = "../../Results/DiffSpa/FileEntry/FileEntry"+dtanno_file[0:-5]+".txt" 
    with open(result_path, 'w') as f:
        json.dump(whofol_results, f, indent=4)     
    
    train_result_path = "../../Results/DiffSpa/FileEntry/FileEntry"+dtanno_file[0:-5]+"_train.txt" 
    with open(train_result_path, 'w') as f:
        json.dump(train_results, f, indent=4)     
        
    spa_result_path = "../../Results/DiffSpa/SpaResults/FileEntry/FileEntry"+dtanno_file[0:-5]+".txt" 
    with open(spa_result_path, 'w') as f:
        json.dump(spa_whofol_results, f, indent=4)    
    
    spa_train_result_path = "../../Results/DiffSpa/SpaResults/FileEntry/FileEntry"+dtanno_file[0:-5]+"_train.txt" 
